...
in
{
  options.services.greetd = {
    ...
    settings = lib.mkOption {
      ...
      example = lib.literalExpression ''
        {
          default_session = {
            command = "''${cfg.package}/bin/agreety --cmd sway";
          };
        }
      '';
      description = ''
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services = builtins.listToAttrs (
      builtins.map (seat: lib.attrsets.nameValuePair "greetd-${seat}" {
        unitConfig = {
          Wants = [
        ...
        serviceConfig =
        let
          seatSettings = cfg.settings // {
            general.seat = seat;
            terminal.vt = if seat == "seat0"
              then cfg.settings.terminal.vt
              else "none";
          };
        in {
          ExecStart = "${cfg.package}/bin/greetd --config ${settingsFormat.generate "greetd-${seat}.toml" seatSettings}";
